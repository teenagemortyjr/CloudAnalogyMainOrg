...
import getRecord from '@salesforce/apex/defaultApex.getRecord' ;
...
export default class ShowRecordLwc extends NavigationMixin(LightningElement) {
    ...
    handleChange(event) {
        ...
        getRecord({ objectType: this.selectedObject, fieldList: this.lstSelected,Id: this.objectRecordId }).then(
            result => {
                ...
                for( let i in result){
                    ...
                    newRecordList = Object.values(result[i]);
    ...
    handleInputChange(event) {
        ...
        this.delayTimeout = setTimeout(() => {
            ...
            search({
                ...
                .then(result => {
                    console.log("inside the LOoop_______________>>>>>>")
                    let stringResult = JSON.stringify(result);
                    let allResult = JSON.parse(stringResult);
                    allResult.forEach(record => {
                        record.FIELD1 = record[this.field];
                        record.FIELD2 = record[this.field1];
                        console.log("Record Field-->"+record.FIELD1)
                        console.log("record.FIELD2-->"+record.FIELD2)
                        if (this.field2) {
                            record.FIELD3 = record[this.field2];
                            console.log("record field 3---->"+ record.FIELD3)
                        } else {
                            record.FIELD3 = '';
                        }
                    });
    ...
    handleSubmit(event){
        ...
        handleRemove = (event) => {
            ...
            console.log("record name isaaaa"+JSON.stringify(this.record))
            console.log("record id in child is+"+JSON.stringify(this.recordId))
